...
class UpdateBannerDialog extends StatelessWidget {
  ManagerHomeController controller;
  int index;

  UpdateBannerDialog({required this.controller,required this. index});
      ...
      child: Padding(
        ...
        child: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Form(
              key: controller.updateBannerFormKey,
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      GetBuilder<ManagerHomeController>(
                        builder: (controller) {
                          if (controller.updatingBannerLoading) {
                            return const CircularProgressIndicator();
                          } else {
                            return CustomButton(
                              onPressed: () async {
                                controller.updateAddBannerButtonStatues();
                                if (controller.updateBannerFormKey.currentState!
                                        .validate() &&
                                    controller.bannerImageName !=
                                        "Choose Image") {
                                  controller.updateBannerData(index: index);
                                }
                              },
                              text: 'Update banner',
                  ...
                  CustomFormField(
                    ...
                    controller: controller.bannerUpdatedName,
                    isPassword: false,
